...
export function AddFollowUpModal({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await createFollowUp(formData)
    setLoading(false)
    if (response.success) {
      onClose()
      window.location.reload()
    } else {
      setError(response.error || 'Failed to add follow-up')
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-app/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-shell rounded-shell w-full max-w-md p-6 shadow-lg border border-border-hairline relative">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-[18px] font-semibold text-text-primary">Add Follow-up</h2>
            <p className="text-[12px] text-text-secondary">Schedule a future action</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[12px] font-medium text-text-secondary mb-1.5">Task Title *</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[12px] font-medium text-text-secondary mb-1.5">Priority</label>
            ...
            <div>
              <label className="block text-[12px] font-medium text-text-secondary mb-1.5">Due Date *</label>
          ...
          {teamMembers.length > 0 && (
            <div>
              <label className="block text-[12px] font-medium text-text-secondary mb-1.5">Assigned Officer</label>
